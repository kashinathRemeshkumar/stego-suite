"""
Mid/Side (M/S) LSB audio steganography.

Idea
----
Any stereo signal (L, R) can be losslessly re-expressed as:
    M = L + R      (Mid  -- the "mono" content, sum of channels)
    S = L - R      (Side -- the stereo/spatial content, difference)
and exactly inverted with integer *floor* division:
    L = (M + S) // 2
    R = (M - S) // 2

The side channel S usually carries much less perceptually important energy
than M (it's zero for mono material, small for centre-panned material), so
it's the classic place to hide data: small changes to S are far less audible
than the same changes to M or to L/R directly.

We hide the *secret* audio's samples in the low-order bits (LSBs) of the
*cover* audio's S channel, sample-for-sample:

    - A short header (80 bits, 1 LSB/sample) stores: how many secret
      samples there are, the secret's sample rate, and how many bits per
      sample (K) were used to embed it.
    - After the header, each cover S-sample has its bottom K bits
      overwritten with the top K bits of one (quantised) secret sample.

Why this is robust to the M/S transform itself (not to further MP3
encoding -- see note at bottom of file):

Because we use integer M=L+R / S=L-R with floor-division inversion, S is
recovered **exactly bit-for-bit** on the decode side even though the
inversion can be off by at most 1 in the Mid channel (i.e. by at most one
16-bit step in L and R, ~-90 dBFS of numerically-introduced noise -- vastly
below anything you could hear, and irrelevant to secret recovery since the
secret only ever lives in S).

IMPORTANT CAVEAT -- read this before assuming your final .mp3 will contain
the secret:
    MP3 is a *lossy* codec. If you take the stego WAV this script produces
    and re-encode it to MP3, the MDCT quantisation stage will scramble
    these LSBs and the secret will NOT be recoverable. This script is
    therefore designed around:
        cover.mp3  --(decode)-->  PCM  --(embed)-->  stego.wav (lossless)
    Your *inputs* can be MP3s; the *stego carrier you distribute/store*
    must stay in a lossless container (WAV/FLAC) for the secret to survive.
    If you truly need the secret to survive a second MP3 encode, you need
    to embed inside the MP3 bitstream itself (parity of quantised MDCT
    coefficients, a la MP3Stego/LAME patches) -- a much larger undertaking
    that hooks the encoder's inner quantisation loop, not a post-hoc PCM
    trick. Happy to scope that separately if you need it.
"""
